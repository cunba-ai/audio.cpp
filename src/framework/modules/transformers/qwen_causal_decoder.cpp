...
QwenCausalDecoderStaticCacheOutputs QwenCausalDecoderModule::build_static_cache_tail(
    ...
    return {
        ...
        hidden,
        logits,
        runtime::TransformerKVCache(cache_steps, step_elems, cache_keys, cache_values),
        std::move(cache_keys),
        std::move(cache_values),
    };
}

QwenCausalDecoderStaticCacheOutputs QwenCausalDecoderModule::build_static_cache_tail_batched(
    core::ModuleBuildContext & ctx,
    ggml_cgraph * graph,
    const core::TensorValue & input,
    const core::TensorValue & positions,
    ...
    if (n_seqs <= 0) {
        throw std::runtime_error("QwenCausalDecoderModule batched static-cache build requires positive n_seqs");
    }
    if (input.shape.rank != 3 || input.shape.dims[0] != n_seqs ||
        input.shape.dims[1] != 1 || input.shape.dims[2] != config_.stack.hidden_size) {
        throw std::runtime_error(
            "QwenCausalDecoderModule batched static-cache input shape must be [n_seqs, 1, hidden]");
    }
    if (static_cast<int64_t>(weights.stack.layers.size()) != config_.stack.layers) {
        throw std::runtime_error("QwenCausalDecoderWeights layer count does not match config");
    }
    ...
    for (const auto & layer : weights.stack.layers) {
        cache_keys.push_back(core::make_tensor(
            ...
            core::TensorShape::from_dims(
                {n_seqs, cache_steps, config_.stack.num_key_value_heads, config_.stack.head_dim})));
        cache_values.push_back(core::make_tensor(
            ctx,
            GGML_TYPE_F32,
            core::TensorShape::from_dims(
    ...
    const auto logits = LinearModule({
                            ...
                            .build(ctx, logits_input, weights.lm_head);
    // Batched caches are owned by the caller's graph; hand them back as a
    // plain struct (the TransformerKVCache member stays empty).
    return {
        std::move(x),
        hidden,
        logits,
