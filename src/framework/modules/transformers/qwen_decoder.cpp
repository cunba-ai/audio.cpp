...
QwenDecoderLayerOutputs QwenDecoderLayerModule::build_with_static_cache_tail_batched(
    ...
    const std::optional<core::TensorValue> & cache_slot,
    const core::TensorValue & attention_mask) const {
    if (input.shape.rank != 3 || input.shape.dims[2] != config_.hidden_size) {
        throw std::runtime_error("Qwen decoder batched static-cache input shape must be [n_seqs, 1, hidden]");
    }
    const int64_t n_seqs = input.shape.dims[0];
    if (input.shape.dims[1] != 1) {
        throw std::runtime_error("Qwen decoder batched static-cache build supports single-token decode per sequence");
    }
    if (cache_key.shape.rank != 4 || cache_key.shape.dims[0] != n_seqs) {
        throw std::runtime_error(
            "Qwen decoder batched static-cache key shape must be [n_seqs, steps, kv_heads, head_dim]");
    }
    const int64_t dim = require_head_dim(config_);
    const int64_t kv_repeats = config_.num_attention_heads / config_.num_key_value_heads;
    ...
    // Layout [1, n_seqs, heads, dim] puts the batch on ggml's ne[2] (the
    // position axis), so ggml_rope_ext's positions [n_seqs] (one position per
    // sequence) matches ne[2] — the same trick transcribe.cpp's batched step
    // uses. After RoPE, transpose back to [n_seqs, 1, heads, dim] for the
    // attention path.
    auto q = core::reshape_tensor(
        ctx, core::ensure_backend_addressable_layout(ctx, qkv.q),
        core::TensorShape::from_dims({1, n_seqs, config_.num_attention_heads, dim}));
    auto k = core::reshape_tensor(
        ctx, core::ensure_backend_addressable_layout(ctx, qkv.k),
        core::TensorShape::from_dims({1, n_seqs, config_.num_key_value_heads, dim}));
    if (config_.use_qk_norm) {
        q = RMSNormModule({dim, config_.rms_norm_eps, true, false}).build(ctx, q, weights.q_norm);
        k = RMSNormModule({dim, config_.rms_norm_eps, true, false}).build(ctx, k, weights.k_norm);
        if (config_.activation_cast.enabled && config_.activation_cast.after_qk_norm) {
    ...
    auto v = core::reshape_tensor(
        ctx, core::ensure_backend_addressable_layout(ctx, qkv.v),
        core::TensorShape::from_dims({1, n_seqs, config_.num_key_value_heads, dim}));
    ...
    q = TransposeModule({{1, 0, 2, 3}, q.shape.rank}).build(ctx, q);
    k = TransposeModule({{1, 0, 2, 3}, k.shape.rank}).build(ctx, k);
    v = TransposeModule({{1, 0, 2, 3}, v.shape.rank}).build(ctx, v);
    ...
    if (config_.runtime.static_cache.update_mode == QwenDecoderStaticCacheUpdateMode::DirectSetRows) {
        ...
        throw std::runtime_error("Qwen decoder batched static-cache build requires DirectSetRows update mode");
    }
    ...
    context = core::reshape_tensor(
        ctx,
        context,
        core::TensorShape::from_dims({n_seqs, 1, config_.num_attention_heads * dim}));
