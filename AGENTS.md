...
| `capi/` | C ABI shared library `libaudiocpp` (`include/audiocpp.h`, `src/audiocpp_capi.cpp`, `README.md`) — gated by `AUDIOCPP_BUILD_CAPI`. Exposes 42 functions: model lifecycle, TTS/ASR/VAD/Diar/Align/SEP/VC, text→audio generation (`audiocpp_generate`) + audio→MIDI transcription (`audiocpp_midi` / `audiocpp_midi_from_wav` for WAV bytes), streaming (chunk-push), device enumeration, model inspection, WAV I/O, artifact types. CI builds 6 platform/backend variants and uploads to OSS. |
