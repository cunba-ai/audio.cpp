...
public:
    DecodeGraphBatched(std::shared_ptr<TextDecoderWeightsRuntime> runtime,
        ...
        : runtime_(std::move(runtime)), cache_steps_(cache_steps),
          n_seqs_(n_seqs) {
        if (cache_steps_ <= 0) {
            throw std::runtime_error("Higgs Audio STT batched decoder requires positive cache length");
        }
        if (n_seqs_ <= 0) {
            throw std::runtime_error("Higgs Audio STT batched decoder requires positive n_seqs");
        }
        const auto build_start = Clock::now();
        ggml_init_params params{graph_arena_bytes, nullptr, true};
        ctx_.reset(ggml_init(params));
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize Higgs Audio STT batched decoder graph context");
        }
        const auto & config = runtime_->assets().config.text_decoder;
        const auto & weights = runtime_->weights();
        core::ModuleBuildContext ctx{ctx_.get(), "higgs_audio_stt.text_decoder.decode.batched",
        ...
        if (keys_.size() != values_.size() ||
            ...
            throw std::runtime_error("Higgs Audio STT batched decoder layer count mismatch");
        }
        logits_ = decoder_out.logits.tensor;
        ggml_set_output(logits_);
        ggml_build_forward_expand(graph_, logits_);
        buffer_ = ggml_backend_alloc_ctx_tensors(ctx_.get(), runtime_->backend());
        if (buffer_ == nullptr) {
            throw std::runtime_error("failed to allocate Higgs Audio STT batched decoder graph");
        }
        attention_mask_values_.assign(
            static_cast<size_t>(cache_steps_) * static_cast<size_t>(n_seqs_),
            ggml_fp32_to_fp16(-INFINITY));
        debug::timing_log_scalar("higgs_audio_stt.text_decoder.decode_batched.graph.build_ms",
                                 engine::debug::elapsed_ms(build_start, Clock::now()));
        debug::trace_log_scalar("higgs_audio_stt.text_decoder.decode_batched_cache_steps",
                                cache_steps_);
    }
    ...
    void import_state(int64_t seq, const runtime::TransformerKVState & state) {
        ...
        if (valid > cache_steps_) {
            throw std::runtime_error("Higgs Audio STT batched decoder import exceeds cache capacity");
        }
        ...
        for (size_t layer = 0; layer < keys_.size(); ++layer) {
            ...
            if (src.key.size() != src.value.size() ||
                ...
                throw std::runtime_error("Higgs Audio STT batched decoder import size mismatch");
            }
    ...
    std::vector<float> run_step(const std::vector<int32_t> & tokens,
        ...
        if (static_cast<int64_t>(tokens.size()) != n_seqs_ ||
            ...
            throw std::runtime_error("Higgs Audio STT batched decoder step input size mismatch");
        }
        for (int64_t b = 0; b < n_seqs_; ++b) {
            if (slots[static_cast<size_t>(b)] >= cache_steps_) {
                throw std::runtime_error("Higgs Audio STT batched decoder cache exhausted");
            }
        }
        ...
        ggml_backend_synchronize(runtime_->backend());
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("Higgs Audio STT batched decoder step graph compute failed");
        }
...
struct HiggsAudioSTTTextDecoderRuntime::Impl {
    ...
    std::vector<HiggsAudioSTTGeneratedTokens> generate_batch(
        ...
        if (n > 32) {
            throw std::runtime_error("Higgs Audio STT batched decoder supports at most 32 utterances");
        }
        if (options.max_new_tokens <= 0) {
            throw std::runtime_error("Higgs Audio STT max_new_tokens must be positive");
        }
        if (audio_embeddings.size() != n) {
            throw std::runtime_error("Higgs Audio STT batched decoder input count mismatch");
        }
        ...
        for (size_t b = 0; b < n; ++b) {
            ...
            if (prompt.input_ids.empty()) {
                throw std::runtime_error("Higgs Audio STT batched decoder prompt is empty");
            }
            ...
            if (steps + options.max_new_tokens > config.max_position_embeddings) {
                throw std::runtime_error("Higgs Audio STT batched decoder request exceeds max_position_embeddings");
            }
        ...
        // ---- Lockstep generation ----
        std::vector<HiggsAudioSTTGeneratedTokens> out(n);
...
std::vector<HiggsAudioSTTGeneratedTokens> HiggsAudioSTTTextDecoderRuntime::generate_batch(
    const std::vector<HiggsAudioSTTPrompt> & prompts,
    const std::vector<HiggsAudioSTTAudioEmbeddings> & audio_embeddings,
    const HiggsAudioSTTGenerationOptions & options) {
