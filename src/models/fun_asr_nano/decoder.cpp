...
int64_t head_dim(const FunAsrNanoTextConfig &config) {
  if (config.attention_heads <= 0 || config.key_value_heads <= 0 ||
    ...
    throw std::runtime_error(
        "Fun-ASR-Nano decoder attention config is invalid");
  }
...
prompt_embeddings(core::ModuleBuildContext &ctx,
  ...
  auto ids = core::wrap_tensor(
      token_ids, core::TensorShape::from_dims({prompt_steps}), GGML_TYPE_I32);
  ...
  if (audio_tokens > 0) {
    ...
    auto positions = core::wrap_tensor(
        audio_positions, core::TensorShape::from_dims({audio_tokens}),
        GGML_TYPE_I64);
    x = core::wrap_tensor(
...
TextDecoderWeights load_weights(const FunAsrNanoAssets &assets,
  ...
  weights.token_embedding = weights.store->load_tensor(
      source, "model.language_model.embed_tokens.weight", storage_type,
  ...
  for (int64_t layer = 0; layer < config.layers; ++layer) {
    ...
    w.q_norm = weights.store->load_f32_tensor(
        source, prefix + ".self_attn.q_norm.weight", {dim});
    w.k_norm = weights.store->load_f32_tensor(
  ...
  weights.norm = weights.store->load_f32_tensor(
      source, "model.language_model.norm.weight", {config.hidden_size});
...
require_assets(std::shared_ptr<const FunAsrNanoAssets> assets) {
  if (assets == nullptr) {
    throw std::runtime_error("Fun-ASR-Nano decoder requires assets");
  }
  return assets;
}
...
public:
  TextDecoderWeightsRuntime(std::shared_ptr<const FunAsrNanoAssets> assets,
    ...
    if (backend_ == nullptr) {
      throw std::runtime_error(
          "Fun-ASR-Nano decoder backend is not initialized");
    }
  }
...
public:
  PrefillGraph(std::shared_ptr<TextDecoderWeightsRuntime> runtime,
    ...
    if (prompt_steps_ <= 0) {
      throw std::runtime_error(
          "Fun-ASR-Nano decoder prefill requires positive prompt length");
    }
    if (audio_tokens_ < 0 || audio_tokens_ > prompt_steps_) {
      throw std::runtime_error(
          "Fun-ASR-Nano decoder prefill audio token count is invalid");
    }
    ...
    if (ctx_ == nullptr) {
      throw std::runtime_error(
          "failed to initialize Fun-ASR-Nano decoder prefill graph context");
    }
    const auto &config = runtime_->assets().config.text;
    const auto &weights = runtime_->weights();
    core::ModuleBuildContext ctx{ctx_.get(), "fun_asr_nano.decoder.prefill",
                                 runtime_->backend_type()};
    token_ids_ = ggml_new_tensor_1d(ctx_.get(), GGML_TYPE_I32, prompt_steps_);
    audio_embeddings_ =
        ggml_new_tensor_2d(ctx_.get(), GGML_TYPE_F32, config.hidden_size,
                           std::max<int64_t>(audio_tokens_, 1));
    audio_positions_ = ggml_new_tensor_1d(ctx_.get(), GGML_TYPE_I64,
                                          std::max<int64_t>(audio_tokens_, 1));
    auto x =
        prompt_embeddings(ctx, weights, config, token_ids_, audio_embeddings_,
                          audio_positions_, prompt_steps_, audio_tokens_);
    positions_ = ggml_new_tensor_1d(ctx_.get(), GGML_TYPE_I32, prompt_steps_);
    auto positions = core::wrap_tensor(
        positions_, core::TensorShape::from_dims({prompt_steps_}),
        GGML_TYPE_I32);
    ...
    for (const auto &layer : decoder_out.state.layers) {
      if (!layer.key.has_value() || !layer.value.has_value()) {
        throw std::runtime_error(
            "Fun-ASR-Nano decoder prefill decoder did not return K/V state");
      }
      auto *key_readback =
    ...
    debug::timing_log_scalar(
        "fun_asr_nano.decoder.prefill.graph.build_ms",
        engine::debug::elapsed_ms(build_start, Clock::now()));
    debug::trace_log_scalar("fun_asr_nano.decoder.prefill_prompt_steps",
                            prompt_steps_);
  }
  ...
  PrefillOutput run(const std::vector<int32_t> &token_ids,
    ...
    if (static_cast<int64_t>(token_ids.size()) != prompt_steps_) {
      throw std::runtime_error(
          "Fun-ASR-Nano decoder prefill token id count mismatch");
    }
    if (static_cast<int64_t>(audio_embeddings.size()) !=
        audio_tokens_ * config.hidden_size) {
      throw std::runtime_error(
          "Fun-ASR-Nano decoder prefill audio embedding size mismatch");
    }
    if (static_cast<int64_t>(audio_positions.size()) != audio_tokens_) {
      throw std::runtime_error(
          "Fun-ASR-Nano decoder prefill audio position count mismatch");
    }
    auto timing_start = Clock::now();
    ggml_backend_tensor_set(token_ids_, token_ids.data(), 0,
                            token_ids.size() * sizeof(int32_t));
    if (audio_tokens_ > 0) {
      std::vector<int64_t> positions(audio_positions.begin(),
                                     audio_positions.end());
      ggml_backend_tensor_set(audio_embeddings_, audio_embeddings.data(), 0,
                              audio_embeddings.size() * sizeof(float));
      ggml_backend_tensor_set(audio_positions_, positions.data(), 0,
                              positions.size() * sizeof(int64_t));
    }
    debug::timing_log_scalar(
        "fun_asr_nano.decoder.prefill_input_upload_ms",
    ...
    debug::timing_log_scalar(
        "fun_asr_nano.decoder.prefill.graph.compute_ms",
        engine::debug::elapsed_ms(timing_start, Clock::now()));
    if (status != GGML_STATUS_SUCCESS) {
      throw std::runtime_error(
          "Fun-ASR-Nano decoder prefill graph compute failed");
    }
    PrefillOutput out;
    out.logits.resize(static_cast<size_t>(config.vocab_size));
    timing_start = Clock::now();
    ggml_backend_tensor_get(logits_, out.logits.data(), 0,
    ...
    const size_t layer_values = static_cast<size_t>(
        prompt_steps_ * config.key_value_heads * head_dim(config));
    for (size_t layer = 0; layer < keys_.size(); ++layer) {
      ...
      if (state.key.size() != layer_values ||
          state.value.size() != layer_values) {
        throw std::runtime_error(
            "Fun-ASR-Nano decoder prefill K/V export size mismatch");
      }
    }
    debug::timing_log_scalar(
        "fun_asr_nano.decoder.prefill_output_read_ms",
        engine::debug::elapsed_ms(timing_start, Clock::now()));
    return out;
  }
...
public:
  DecodeGraph(std::shared_ptr<TextDecoderWeightsRuntime> runtime,
    ...
    if (real_cache_steps_ <= 0) {
      throw std::runtime_error(
          "Fun-ASR-Nano decoder decode requires positive cache length");
    }
    const auto build_start = Clock::now();
    ggml_init_params params{graph_arena_bytes, nullptr, true};
    ctx_.reset(ggml_init(params));
    if (ctx_ == nullptr) {
      throw std::runtime_error(
          "failed to initialize Fun-ASR-Nano decoder decode graph context");
    }
    const auto &config = runtime_->assets().config.text;
    const auto &weights = runtime_->weights();
    core::ModuleBuildContext ctx{ctx_.get(), "fun_asr_nano.decoder.decode",
                                 runtime_->backend_type()};
    token_id_ = ggml_new_tensor_1d(ctx_.get(), GGML_TYPE_I32, 1);
    ggml_set_input(token_id_);
    auto token_id = core::wrap_tensor(
        token_id_, core::TensorShape::from_dims({1}), GGML_TYPE_I32);
    auto x = modules::EmbeddingModule({config.vocab_size, config.hidden_size})
                 .build(ctx, token_id, weights.token_embedding);
    x = core::reshape_tensor(
        ctx, x, core::TensorShape::from_dims({1, 1, config.hidden_size}));
    positions_ = ggml_new_tensor_1d(ctx_.get(), GGML_TYPE_I32, 1);
    ggml_set_input(positions_);
    auto positions = core::wrap_tensor(
        positions_, core::TensorShape::from_dims({1}), GGML_TYPE_I32);
    cache_slot_ = ggml_new_tensor_1d(ctx_.get(), GGML_TYPE_I32, 1);
    ggml_set_input(cache_slot_);
    auto cache_slot = core::wrap_tensor(
        cache_slot_, core::TensorShape::from_dims({1}), GGML_TYPE_I32);
    attention_mask_ =
    ...
    auto attention_mask = core::wrap_tensor(
        attention_mask_, core::TensorShape::from_dims({1, 1, 1, cache_steps_}),
        GGML_TYPE_F16);
    graph_ = ggml_new_graph_custom(ctx_.get(), 65536, false);
    auto decoder_out =
        modules::QwenCausalDecoderModule(make_qwen_decoder_config(config))
    ...
    if (buffer_ == nullptr) {
      throw std::runtime_error(
          "failed to allocate Fun-ASR-Nano decoder decode graph");
    }
    attention_mask_values_.assign(static_cast<size_t>(cache_steps_),
                                  ggml_fp32_to_fp16(-INFINITY));
    debug::timing_log_scalar(
        "fun_asr_nano.decoder.decode.graph.build_ms",
        engine::debug::elapsed_ms(build_start, Clock::now()));
    debug::trace_log_scalar("fun_asr_nano.decoder.decode_cache_steps",
                            real_cache_steps_);
  }
  ...
  std::vector<float> run_step(int32_t token) {
    ...
    if (step_cache_.valid_steps() >= real_cache_steps_) {
      throw std::runtime_error("Fun-ASR-Nano decoder decode cache exhausted");
    }
    ...
    if (status != GGML_STATUS_SUCCESS) {
      throw std::runtime_error(
          "Fun-ASR-Nano decoder decode graph compute failed");
    }
...
// sequences share one step graph. Each sequence has its own KV slab inside
...
// [n_seqs, vocab] logits per step. The per-utterance prefill K/V is imported
// into each sequence's slab before the step loop.
class DecodeGraphBatched {
public:
  DecodeGraphBatched(std::shared_ptr<TextDecoderWeightsRuntime> runtime,
    ...
    if (cache_steps_ <= 0) {
      throw std::runtime_error(
          "Fun-ASR-Nano batched decoder requires positive cache length");
    }
    if (n_seqs_ <= 0) {
      throw std::runtime_error(
          "Fun-ASR-Nano batched decoder requires positive n_seqs");
    }
    const auto build_start = Clock::now();
    ggml_init_params params{graph_arena_bytes, nullptr, true};
    ctx_.reset(ggml_init(params));
    if (ctx_ == nullptr) {
      throw std::runtime_error(
          "failed to initialize Fun-ASR-Nano batched decoder graph context");
    }
    const auto &config = runtime_->assets().config.text;
    const auto &weights = runtime_->weights();
    core::ModuleBuildContext ctx{ctx_.get(),
                                 "fun_asr_nano.decoder.decode.batched",
                                 runtime_->backend_type()};
    ...
    if (keys_.size() != values_.size() ||
        keys_.size() != static_cast<size_t>(config.layers)) {
      throw std::runtime_error(
          "Fun-ASR-Nano batched decoder layer count mismatch");
    }
    logits_ = decoder_out.logits.tensor;
    ggml_set_output(logits_);
    ggml_build_forward_expand(graph_, logits_);
    ...
    if (buffer_ == nullptr) {
      throw std::runtime_error(
          "failed to allocate Fun-ASR-Nano batched decoder graph");
    }
    attention_mask_values_.assign(
        static_cast<size_t>(cache_steps_) * static_cast<size_t>(n_seqs_),
        ggml_fp32_to_fp16(-INFINITY));
    debug::timing_log_scalar(
        "fun_asr_nano.decoder.decode_batched.graph.build_ms",
        engine::debug::elapsed_ms(build_start, Clock::now()));
    debug::trace_log_scalar(
        "fun_asr_nano.decoder.decode_batched_cache_steps", cache_steps_);
  }
  ...
  void import_state(int64_t seq, const runtime::TransformerKVState &state) {
    ...
    if (valid > cache_steps_) {
      throw std::runtime_error(
          "Fun-ASR-Nano batched decoder import exceeds cache capacity");
    }
    ...
    for (size_t layer = 0; layer < keys_.size(); ++layer) {
      ...
      if (src.key.size() != src.value.size() ||
        ...
        throw std::runtime_error(
            "Fun-ASR-Nano batched decoder import size mismatch");
      }
      ggml_backend_tensor_set(keys_[layer].tensor, src.key.data(), slab, bytes);
      ggml_backend_tensor_set(values_[layer].tensor, src.value.data(), slab,
                              bytes);
    }
  }

  // One lockstep step. tokens/positions/slots are per-sequence; visible[b] is
  // each sequence's visible prefix length (its own prefill length + steps so
  // far). Returns [n_seqs, vocab] logits.
  std::vector<float> run_step(const std::vector<int32_t> &tokens,
    ...
    if (static_cast<int64_t>(tokens.size()) != n_seqs_ ||
      ...
      throw std::runtime_error(
          "Fun-ASR-Nano batched decoder step input size mismatch");
    }
    for (int64_t b = 0; b < n_seqs_; ++b) {
      if (slots[static_cast<size_t>(b)] >= cache_steps_) {
        throw std::runtime_error(
            "Fun-ASR-Nano batched decoder cache exhausted");
      }
    }
    ...
    // FastKVSetRows indexes the flat [n_seqs * cache_steps, row_elems] cache
    // with row = batch * cache_steps + slot, so per-sequence slots must carry
    // the batch offset.
    ...
    if (status != GGML_STATUS_SUCCESS) {
      throw std::runtime_error(
          "Fun-ASR-Nano batched decoder step graph compute failed");
    }
...
struct FunAsrNanoDecoderRuntime::Impl {
  ...
  void validate_prompt_audio(
      ...
      const FunAsrNanoAdaptorEmbeddings &audio_embeddings) const {
    const auto &config = weights->assets().config.text;
    if (audio_embeddings.hidden_size != config.hidden_size) {
      throw std::runtime_error(
          "Fun-ASR-Nano audio embedding hidden size mismatch");
    }
    if (audio_embeddings.tokens !=
        static_cast<int64_t>(prompt.audio_token_positions.size())) {
      throw std::runtime_error("Fun-ASR-Nano audio embedding token count does "
                               "not match prompt placeholders");
    }
    if (static_cast<int64_t>(audio_embeddings.values.size()) !=
        audio_embeddings.tokens * config.hidden_size) {
      throw std::runtime_error(
          "Fun-ASR-Nano audio embedding value count mismatch");
    }
    std::vector<int32_t> expected_audio_positions;
    for (size_t index = 0; index < prompt.input_ids.size(); ++index) {
      if (prompt.input_ids[index] == weights->assets().config.audio_token_id) {
        expected_audio_positions.push_back(static_cast<int32_t>(index));
      }
    }
    if (prompt.audio_token_positions != expected_audio_positions) {
      throw std::runtime_error(
          "Fun-ASR-Nano audio positions must cover every audio placeholder in "
          "prompt order");
    }
    if (prompt.attention_mask.size() != prompt.input_ids.size() ||
        std::any_of(prompt.attention_mask.begin(), prompt.attention_mask.end(),
                    [](int32_t value) { return value != 1; })) {
      throw std::runtime_error(
          "Fun-ASR-Nano decoder requires an unpadded prompt mask");
    }
  }
  ...
  generate(const FunAsrNanoPrompt &prompt,
    ...
    if (prompt.input_ids.empty()) {
      throw std::runtime_error("Fun-ASR-Nano decoder prompt is empty");
    }
    if (options.max_new_tokens <= 0) {
      throw std::runtime_error("Fun-ASR-Nano max_new_tokens must be positive");
    }
    const int64_t prompt_steps = static_cast<int64_t>(prompt.input_ids.size());
    if (prompt_steps > config.max_position_embeddings ||
        options.max_new_tokens >
            config.max_position_embeddings - prompt_steps) {
      throw std::runtime_error(
          "Fun-ASR-Nano decoder request exceeds max_position_embeddings");
    }
    auto timing_start = Clock::now();
    validate_prompt_audio(prompt, audio_embeddings);
    debug::timing_log_scalar(
        "fun_asr_nano.decoder.prompt_prepare_ms",
        engine::debug::elapsed_ms(timing_start, Clock::now()));
    if (prefill_graph == nullptr ||
      ...
      debug::timing_log_scalar("fun_asr_nano.decoder.prefill.graph.build_ms",
                               0.0);
      debug::trace_log_scalar("fun_asr_nano.decoder.prefill_prompt_steps",
                              prompt_steps);
    }
    timing_start = Clock::now();
    auto prefill = prefill_graph->run(prompt.input_ids, audio_embeddings.values,
                                      prompt.audio_token_positions);
    debug::timing_log_scalar(
        "fun_asr_nano.decoder.prefill_total_ms",
        engine::debug::elapsed_ms(timing_start, Clock::now()));
    const int64_t required_cache_steps =
        prompt_steps + std::max<int64_t>(options.max_new_tokens - 1, 0);
    ...
    for (int64_t step = 0; step < options.max_new_tokens; ++step) {
      ...
      if (decode_graph == nullptr) {
        throw std::runtime_error(
            "Fun-ASR-Nano decoder decode graph is not initialized");
      }
      logits = decode_graph->run_step(token);
    }
    debug::timing_log_scalar(
        "fun_asr_nano.decoder.decode_total_ms",
        engine::debug::elapsed_ms(timing_start, Clock::now()));
    return out;
  }

  std::vector<FunAsrNanoGeneratedTokens> generate_batch(
      const std::vector<FunAsrNanoPrompt> &prompts,
    ...
    if (audio_embeddings.size() != n) {
      throw std::runtime_error(
          "Fun-ASR-Nano batched decoder prompt/audio count mismatch");
    }
    if (!token_callbacks.empty() && token_callbacks.size() != n) {
      throw std::runtime_error(
          "Fun-ASR-Nano batched decoder callback count mismatch");
    }
    if (options.max_new_tokens <= 0) {
      throw std::runtime_error("Fun-ASR-Nano max_new_tokens must be positive");
    }
    ...
    for (size_t b = 0; b < n; ++b) {
      ...
      if (prompt.input_ids.empty()) {
        throw std::runtime_error("Fun-ASR-Nano batched decoder prompt is empty");
      }
      const int64_t steps = static_cast<int64_t>(prompt.input_ids.size());
      if (steps > config.max_position_embeddings ||
          options.max_new_tokens > config.max_position_embeddings - steps) {
        throw std::runtime_error(
    ...
    // ---- Lockstep generation ----
    std::vector<FunAsrNanoGeneratedTokens> out(n);
    ...
    for (size_t b = 0; b < n; ++b) {
      ...
      if (is_eos(config, tok)) {
        ...
        if (!token_callbacks.empty() && token_callbacks[b]) {
          token_callbacks[b](out[b]);
        }
      }
      tokens[b] = tok;
    }
    for (int64_t step = 1; step < options.max_new_tokens && active > 0; ++step) {
      ...
      for (size_t b = 0; b < n; ++b) {
        ...
        const float *row = logits_batch.data() + b * vocab;
        const int32_t tok = argmax_index_ptr(row, vocab);
        if (is_eos(config, tok)) {
          finished[b] = 1;
          --active;
        } else {
    ...
    debug::timing_log_scalar(
        "fun_asr_nano.decoder.decode_batched_total_ms",
        engine::debug::elapsed_ms(begin, Clock::now()));
    return out;
  }
...
std::vector<FunAsrNanoGeneratedTokens> FunAsrNanoDecoderRuntime::generate_batch(
    const std::vector<FunAsrNanoPrompt> &prompts,
    const std::vector<FunAsrNanoAdaptorEmbeddings> &audio_embeddings,
    const FunAsrNanoGenerationOptions &options,
    const std::vector<FunAsrNanoTokenCallback> &token_callbacks) {
