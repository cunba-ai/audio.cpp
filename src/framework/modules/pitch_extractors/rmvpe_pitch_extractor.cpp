...
struct RmvpePitchExtractorWeights {
    std::shared_ptr<engine::core::ExecutionContext> execution_context;
    std::shared_ptr<engine::core::BackendWeightStore> store;
    std::unordered_map<std::string, TensorValue> tensors;
};

RmvpePitchExtractorWeights load_rmvpe_weights(
    std::shared_ptr<const engine::assets::TensorSource> source,
    engine::core::BackendConfig backend,
    engine::assets::TensorStorageType storage_type,
    const RmvpePitchExtractorConfig & config) {
    if (source == nullptr) {
        throw std::runtime_error("RMVPE requires weights");
    }
    RmvpePitchExtractorWeights weights;
...
const TensorValue & require_tensor(const RmvpePitchExtractorWeights & weights, const std::string & name) {
    const auto it = weights.tensors.find(name);
    if (it == weights.tensors.end()) {
        throw std::runtime_error("RMVPE missing tensor: " + name);
    }
    return it->second;
}
...
engine::modules::Conv2dWeights conv2d_weights(
    const RmvpePitchExtractorWeights & weights,
...
TensorValue batch_norm2d(
    ...
    auto normalized = engine::core::wrap_tensor(
        ...
        input.shape,
        GGML_TYPE_F32);
    return engine::modules::AddModule{}.build(ctx, engine::modules::MulModule{}.build(ctx, normalized, weight_rep), bias_rep);
}
...
TensorValue conv_bn_relu(
    ...
    return engine::modules::ReluModule{}.build(ctx, x);
}
...
TensorValue conv_block_res(
    ...
    return engine::modules::AddModule{}.build(ctx, y, residual);
}
...
TensorValue conv_transpose2d_pytorch_2x(
    ...
    auto full = engine::core::wrap_tensor(
        ...
        GGML_TYPE_F32);
    full = engine::modules::SliceModule({2, 1, input.shape.dims[2] * 2}).build(ctx, full);
    full = engine::modules::SliceModule({3, 1, input.shape.dims[3] * 2}).build(ctx, full);
    return full;
}
...
RmvpeGruOutputs build_gru_chunk_graph(
    ...
    for (int64_t index = 0; index < frames; ++index) {
        ...
        const auto xi = engine::modules::SliceModule({0, t, 1}).build(ctx, projected_input);
        ...
        const auto i_r = engine::modules::SliceModule({1, 0, kRmvpeHiddenDim}).build(ctx, xi);
        const auto i_z = engine::modules::SliceModule({1, kRmvpeHiddenDim, kRmvpeHiddenDim}).build(ctx, xi);
        const auto i_n = engine::modules::SliceModule({1, kRmvpeHiddenDim * 2, kRmvpeHiddenDim}).build(ctx, xi);
        const auto h_r = engine::modules::SliceModule({1, 0, kRmvpeHiddenDim}).build(ctx, hi);
        const auto h_z = engine::modules::SliceModule({1, kRmvpeHiddenDim, kRmvpeHiddenDim}).build(ctx, hi);
        const auto h_n = engine::modules::SliceModule({1, kRmvpeHiddenDim * 2, kRmvpeHiddenDim}).build(ctx, hi);
        const auto reset = engine::modules::SigmoidModule{}.build(ctx, engine::modules::AddModule{}.build(ctx, i_r, h_r));
        const auto update = engine::modules::SigmoidModule{}.build(ctx, engine::modules::AddModule{}.build(ctx, i_z, h_z));
        const auto candidate = engine::modules::TanhModule{}.build(
            ctx,
            engine::modules::AddModule{}.build(ctx, i_n, engine::modules::MulModule{}.build(ctx, reset, h_n)));
        const auto updated = engine::modules::AddModule{}.build(
            ctx,
            candidate,
            engine::modules::MulModule{}.build(ctx, update, sub(ctx, hidden, candidate)));
        const auto keep_t = engine::modules::SliceModule({0, t, 1}).build(ctx, keep);
        ...
        hidden = engine::modules::AddModule{}.build(
            ctx,
            hidden,
            engine::modules::MulModule{}.build(ctx, keep_rep, sub(ctx, updated, hidden)));
...
TensorValue build_rmvpe_head_graph(
    ...
    return engine::modules::SigmoidModule{}.build(ctx, logits);
}
...
std::vector<float> pad_rmvpe_mel_time_axis(
    ...
    if (static_cast<int64_t>(mel.size()) != kRmvpeMelBins * frames || frames <= 0 || padded_frames < frames) {
        throw std::runtime_error("RMVPE mel padding shape mismatch");
    }
...
void copy_mel_window(
    ...
    if (start < 0 || frames < 0 || start + frames > padded_frames) {
        throw std::runtime_error("RMVPE mel window is out of range");
    }
...
struct RmvpePitchExtractorComponent::State {
    ~State() {
    ...
    void ensure_feature_graph(const RmvpePitchExtractorWeights & weights, int64_t frames) {
        if (frames <= 0 || frames % 32 != 0) {
            throw std::runtime_error("RMVPE feature graph requires positive 32-aligned frame count");
        }
        ...
        const std::string graph_name = config.trace_prefix + ".feature";
        ...
        if (feature.gallocr == nullptr ||
            ...
            release_feature_graph();
            throw std::runtime_error("failed to allocate RMVPE feature graph tensors");
        }
    ...
    void ensure_gru_graph(const RmvpePitchExtractorWeights & weights, GruGraph & target, const std::string & suffix, bool reverse_graph) {
        ...
        const std::string graph_name = config.trace_prefix + (reverse_graph ? ".gru_reverse" : ".gru_forward");
        engine::core::ModuleBuildContext build_ctx{
            target.ctx,
            graph_name.c_str(),
            weights.execution_context->backend_type()};
    ...
    void ensure_head_graph(const RmvpePitchExtractorWeights & weights) {
        if (head.ctx != nullptr) {
            return;
        }
        if (weights.execution_context == nullptr) {
        ...
        if (head.ctx == nullptr) {
            throw std::runtime_error("failed to initialize RMVPE head graph context");
        }
        const std::string graph_name = config.trace_prefix + ".head";
        engine::core::ModuleBuildContext build_ctx{
            head.ctx,
            graph_name.c_str(),
            weights.execution_context->backend_type()};
        ...
        if (head.gallocr == nullptr ||
            ...
            release_head_graph();
            throw std::runtime_error("failed to allocate RMVPE head graph tensors");
        }
    }

    void ensure_graphs(const RmvpePitchExtractorWeights & weights, int64_t feature_frames) {
...
RmvpePitchExtractorComponent::RmvpePitchExtractorComponent(
    std::shared_ptr<const engine::assets::TensorSource> source,
    engine::core::BackendConfig backend,
    engine::assets::TensorStorageType storage_type,
    RmvpePitchExtractorConfig config)
    ...
    state_->weights = load_rmvpe_weights(std::move(source), std::move(backend), storage_type, state_->config);
}

RmvpePitchExtractorComponent::~RmvpePitchExtractorComponent() = default;
RmvpePitchExtractorComponent::RmvpePitchExtractorComponent(RmvpePitchExtractorComponent &&) noexcept = default;
RmvpePitchExtractorComponent & RmvpePitchExtractorComponent::operator=(RmvpePitchExtractorComponent &&) noexcept = default;

std::vector<float> RmvpePitchExtractorComponent::infer_16k_mono(
    const std::vector<float> & waveform_16k,
    float threshold,
    size_t threads) const {
    if (state_ == nullptr) {
        throw std::runtime_error("RMVPE is not initialized");
    }
    ...
    engine::debug::timing_log_scalar(
        state_->config.trace_prefix + ".mel_ms",
    ...
    engine::debug::timing_log_scalar(
        state_->config.trace_prefix + ".prepare_ms",
        engine::debug::elapsed_ms(timing_start, timing_end));
    timing_start = timing_end;
    ...
    for (int64_t active_start = 0; active_start < mel.padded_frames; active_start += feature_step) {
        ...
        if (engine::core::compute_backend_graph(backend, state_->feature.graph) != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("ggml_backend_graph_compute failed for RMVPE feature graph");
        }
    ...
    for (int64_t end = mel.padded_frames; end > 0;) {
        ...
        if (salience_rows > 0) {
            ...
            if (engine::core::compute_backend_graph(backend, state_->head.graph) != GGML_STATUS_SUCCESS) {
                throw std::runtime_error("ggml_backend_graph_compute failed for RMVPE head graph");
            }
    ...
    timing_end = std::chrono::steady_clock::now();
    engine::debug::timing_log_scalar(
        state_->config.trace_prefix + ".compute_ms",
        engine::debug::elapsed_ms(timing_start, timing_end));
    timing_start = timing_end;
    auto f0 = decode_rmvpe_salience(salience, mel.frames, threshold);
    timing_end = std::chrono::steady_clock::now();
    engine::debug::timing_log_scalar(
        state_->config.trace_prefix + ".decode_ms",
        engine::debug::elapsed_ms(timing_start, timing_end));
    return f0;
}
