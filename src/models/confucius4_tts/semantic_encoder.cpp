...
core::TensorValue wav2vec2bert_attention(
    ...
    const core::TensorValue & distance_ids,
    const ConfuciusWav2Vec2BertAttentionWeights & weights) {
...
core::TensorValue wav2vec2bert_conv(
    ...
    const core::TensorValue & input,
    const core::TensorValue & keep_mask,
    const ConfuciusWav2Vec2BertConvWeights & weights) {
...
core::TensorValue wav2vec2bert_layer(
    ...
    const core::TensorValue & attention_mask,
    const core::TensorValue & distance_ids,
    const ConfuciusWav2Vec2BertLayerWeights & weights) {
...
ConfuciusWav2Vec2BertAttentionWeights load_attention(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    const std::string & prefix,
    engine::assets::TensorStorageType storage_type) {
    ConfuciusWav2Vec2BertAttentionWeights weights;
...
ConfuciusWav2Vec2BertConvWeights load_conv_module(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    const std::string & prefix,
    engine::assets::TensorStorageType storage_type) {
    ConfuciusWav2Vec2BertConvWeights weights;
...
ConfuciusWav2Vec2BertLayerWeights load_layer(
    engine::core::BackendWeightStore & store,
    const engine::assets::TensorSource & source,
    int64_t layer_index,
    ...
    ConfuciusWav2Vec2BertLayerWeights layer;
...
std::vector<float> make_attention_mask(const std::vector<int32_t> & keep_mask, int64_t frames) {
    if (static_cast<int64_t>(keep_mask.size()) != frames) {
        throw std::runtime_error("Confucius4-TTS Wav2Vec2-BERT attention mask length mismatch");
    }
...
class ConfuciusWav2Vec2BertRuntime::Graph {
public:
    Graph(
        engine::core::ExecutionContext & execution,
        std::shared_ptr<const ConfuciusWav2Vec2BertWeights> weights,
        int64_t frames,
        ...
        : execution_(execution),
          ...
          frames_(frames) {
        if (frames_ <= 0) {
            throw std::runtime_error("Confucius4-TTS Wav2Vec2-BERT graph requires positive frame count");
        }
        if (weights_ == nullptr) {
            throw std::runtime_error("Confucius4-TTS Wav2Vec2-BERT graph requires weights");
        }
        ...
        if (ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize Confucius4-TTS Wav2Vec2-BERT graph context");
        }
        ...
        if (input_ctx_ == nullptr) {
            throw std::runtime_error("failed to initialize Confucius4-TTS Wav2Vec2-BERT input context");
        }
        core::ModuleBuildContext ctx{ctx_.get(), "confucius4_tts.wav2vec2bert", execution_.backend_type()};
        core::ModuleBuildContext input_ctx{
            input_ctx_.get(),
            "confucius4_tts.wav2vec2bert.inputs",
        ...
        if (input_buffer_ == nullptr) {
            throw std::runtime_error("failed to allocate Confucius4-TTS Wav2Vec2-BERT input buffer");
        }
        ...
        if (gallocr_ == nullptr ||
            ...
            clear_graph();
            throw std::runtime_error("failed to allocate Confucius4-TTS Wav2Vec2-BERT graph");
        }
        ...
        debug::timing_log_scalar(
            "confucius4_tts.wav2vec2bert.graph.build_ms",
            engine::debug::elapsed_ms(build_start, Clock::now()));
        debug::trace_log_scalar("confucius4_tts.wav2vec2bert.frames", frames_);
    }
    ...
    ConfuciusSemanticEmbedding run(const ConfuciusSemanticFeatureOutput & features) {
        if (features.frames <= 0 || features.frames > frames_ || features.dims != kFeatureDim) {
            throw std::runtime_error("Confucius4-TTS Wav2Vec2-BERT feature shape does not match prepared graph");
        }
        if (static_cast<int64_t>(features.values.size()) != features.frames * kFeatureDim) {
            throw std::runtime_error("Confucius4-TTS Wav2Vec2-BERT feature value count mismatch");
        }
        if (static_cast<int64_t>(features.attention_mask.size()) != features.frames) {
            throw std::runtime_error("Confucius4-TTS Wav2Vec2-BERT feature attention mask count mismatch");
        }
        ...
        debug::timing_log_scalar(
            "confucius4_tts.wav2vec2bert.host_input_prepare_ms",
        ...
        debug::timing_log_scalar(
            "confucius4_tts.wav2vec2bert.input_upload_ms",
            engine::debug::elapsed_ms(timing_start, Clock::now()));
        ...
        debug::timing_log_scalar(
            "confucius4_tts.wav2vec2bert.graph.compute_ms",
            engine::debug::elapsed_ms(timing_start, Clock::now()));
        if (status != GGML_STATUS_SUCCESS) {
            throw std::runtime_error("Confucius4-TTS Wav2Vec2-BERT graph compute failed");
        }
        ConfuciusSemanticEmbedding out;
        ...
        debug::timing_log_scalar(
            "confucius4_tts.wav2vec2bert.output_read_ms",
            engine::debug::elapsed_ms(timing_start, Clock::now()));
        return out;
    }
...
std::shared_ptr<const ConfuciusWav2Vec2BertWeights> load_confucius_wav2vec2bert_weights(
    ...
    auto weights = std::make_shared<ConfuciusWav2Vec2BertWeights>();
    weights->store = std::make_shared<engine::core::BackendWeightStore>(
        backend,
        backend_type,
        "confucius4_tts.wav2vec2bert.weights",
        weight_context_bytes);

    const auto & source = *assets.semantic_encoder_weights;
    ...
    weights->semantic_mean = weights->store->make_from_f32(
        ...
        matmul_storage_type,
        assets.semantic_stats->require_f32("mean", {kHidden}));
    weights->semantic_std = weights->store->make_from_f32(
        engine::core::TensorShape::from_dims({kHidden}),
        matmul_storage_type,
        wav2vec2bert_std(*assets.semantic_stats));

    weights->store->upload();
    assets.semantic_encoder_weights->release_storage();
    assets.semantic_stats->release_storage();
    return weights;
}
...
ConfuciusWav2Vec2BertRuntime::ConfuciusWav2Vec2BertRuntime(
    ...
    weights_ = load_confucius_wav2vec2bert_weights(
        *assets_,
        execution.backend(),
...
void ConfuciusWav2Vec2BertRuntime::prepare(int64_t frames) {
    if (execution_ == nullptr) {
        throw std::runtime_error("Confucius4-TTS Wav2Vec2-BERT runtime execution context is missing");
    }
    if (frames <= 0) {
        throw std::runtime_error("Confucius4-TTS Wav2Vec2-BERT prepare requires positive frames");
    }
...
ConfuciusSemanticEmbedding ConfuciusWav2Vec2BertRuntime::encode(const ConfuciusSemanticFeatureOutput & features) {
    if (graph_ == nullptr || graph_->frames() < features.frames) {
        throw std::runtime_error("Confucius4-TTS Wav2Vec2-BERT graph was not prepared for this reference length");
    }
